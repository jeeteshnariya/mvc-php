...
<div class="sm:flex sm:items-center sm:justify-between">
  <div>
    <div class="flex items-center gap-x-3">
      <h2 class="text-xl font-medium text-gray-800 dark:text-white">Products</h2>
  ...
  <div class="flex items-center mt-4 gap-x-3">
    <form class="relative flex items-center mt-4 md:mt-0" action="<?= set_url('products/index') ?>" method="post">
      <span class="absolute">
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
            <?php foreach ($rows as $product) : ?>
              ...
              <tr>
                <td class="px-4 py-4 text-sm font-medium whitespace-nowrap"><?= $product['id'] ?></td>
                <td class="px-4 py-4 text-sm font-medium whitespace-nowrap"><?= $product['name'] ?></td>
                <td class="px-4 py-4 text-sm font-medium whitespace-nowrap"><?= substr($product['detail'],0,25) ?> ...</td>
                ...
                <td class="px-4 py-4 text-sm font-medium whitespace-nowrap">
                  <img src="<?= $product['image'] ?>" alt="" class="w-[200px]">
                </td>


                <td class="px-4 py-4 text-sm whitespace-nowrap">

                  <a href="<?= set_url(sprintf('products/edit/%s', $product['id'])) ?>" class="text-blue-500 font-bold ">
                    Edit
                  </a>

                  <a href="<?= set_url('products/delete/' . $product['id']) ?>" class="text-red-500   font-bold  ml-1">
                    Delete
                  </a>
...
<div class="mt-6 sm:flex sm:items-center sm:justify-between ">
  ...
  <div class="flex items-center mt-4 gap-x-4 sm:mt-0">
    <?php if ($page_no - 1 !== 0) : ?>
      <a href="<?= set_url('products/index&page=' . $page_no - 1) ?>" class="flex items-center justify-center w-1/2 px-5 py-2 text-sm text-gray-700 capitalize transition-colors duration-200 bg-white border rounded-md sm:w-auto gap-x-2 hover:bg-gray-100 dark:bg-gray-900 dark:text-gray-200 dark:border-gray-700 dark:hover:bg-gray-800">
